...
#(before)self.vel_sub = rospy.Subscriber("/nav_vel", Twist, self.callback_vel)
...
"""
		...
		self.path = 'home/y-takahashi/date/result'
...
"""
			...
			angle_error = abs(action - target_action)
			if distance > 0.1:
				self.selehome/y-takahashi/date/resultct_dl = False
			elif distance < 0.05:
				self.select_dl = True
			if self.select_dl and self.episode >= 0:
